...
const HeroSection = () => {
  //States
  const [changeHeroImageNumber, setChangeHeroImageNumber] = useState<number>(1);
  ...
  useEffect(() => {
    const dynamicallyChangeNumber = setTimeout(() => {
      if (changeHeroImageNumber === 2) {
        setChangeHeroImageNumber(1);
      } else {
        setChangeHeroImageNumber(2);
      }
    }, 5000);
    return () => {
      clearTimeout(dynamicallyChangeNumber);
    };
  }, [changeHeroImageNumber]);
  ...
  return (
    <div className="container">
      ...
      <section
        ...
        } flex flex-col  text-white bg-cover bg-center bg-no-repeat`}
        style={{
          backgroundImage: `url(/HeroImage${changeHeroImageNumber}.png)`,
        }}
      >
        {/* Navbar  */}
        <Navbar />
